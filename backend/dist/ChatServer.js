...
exports.DistributedChatServer = void 0;
...
class DistributedChatServer {
    constructor(port = 3001, serverId = 'server1') {
        this.users = new Map();
        this.groups = new Map();
    ...
    setupRoutes() {
        ...
        this.app.get('/groups/:groupId/users', (req, res) => {
            const group = this.groups.get(req.params.groupId);
            if (!group) {
                res.status(404).json({ error: 'Group not found' });
                return;
        ...
        this.app.get('/messages/:chatId', (req, res) => {
    ...
    setupSocketHandlers() {
        this.io.on('connection', (socket) => {
            ...
            socket.on('user:reconnect', (userId, callback) => {
            ...
            socket.on('group:join', (groupId, callback) => {
                const userId = customSocket.data.userId;
                const user = userId ? this.users.get(userId) : undefined;
            ...
            socket.on('group:delete', (groupId, callback) => {
                const userId = customSocket.data.userId;
                const user = userId ? this.users.get(userId) : undefined;
                const group = this.groups.get(groupId);
                if (!user || !group) {
                ...
                group.members.forEach((memberId) => {
                    ...
                    if (member) {
                        member.groups = member.groups.filter((id) => id !== groupId);
                    }
                });
                this.groups.delete(groupId);
                this.messages.delete(groupId);
            ...
            socket.on('groups:get', (callback) => {
                callback(Array.from(this.groups.values()));
            });
            socket.on('message:group', (data, callback) => {
                const userId = customSocket.data.userId;
                const user = userId ? this.users.get(userId) : undefined;
            ...
            socket.on('messages:more', (data, callback) => {
                let messages = [];
                if (data.isPrivate) {
                    const userId = customSocket.data.userId;
                    const conversationKey = userId ? [userId, data.chatId].sort().join('_') : '';
...
exports.DistributedChatServer = DistributedChatServer;
